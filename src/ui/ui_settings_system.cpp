...
static void on_timezone_change(uint16_t selected) {
    ...
    config_manager.saveSystemSettings();
    LOG_UI("Timezone: UTC%+d", g_state.timezone_offset);
}
...
static void on_factory_reset_click() {
    ...
    if (reset_confirm_count < 3) {
        ...
        LOG_UI("Reset confirmação %d/3", reset_confirm_count);
        // Mostra alerta de confirmação via label temporário
        ui_show_toast("Pressione mais %d vezes para confirmar", 3 - reset_confirm_count);
    } else {
        LOG_WARN("FACTORY RESET EXECUTED!");
        audioDriver.playSound(SOUND_ALERT);
...
static void on_calibrate_battery_click() {
    LOG_UI("Iniciando calibração de bateria...");
    audioDriver.playSound(SOUND_SUCCESS);
    // Calibração: lê tensão atual e ajusta offset no config
    sys_hw.calibrateBattery();
    ui_show_toast("Bateria calibrada!");
}
...
static void create_screen() {
    lv_obj_t *content = ui_create_settings_screen(&scr_system, LV_SYMBOL_SETTINGS " Sistema", on_back_click);

    // === SEÇÃO ENERGIA ===
    ui_create_section_header(content, "Energia");

    ui_create_slider_row(
        content, "Deep Sleep", LV_SYMBOL_POWER, 0, 60, g_state.deep_sleep_minutes, on_deep_sleep_change
    );

    ui_create_switch_row(
        content, "Wake on Touch", LV_SYMBOL_EDIT, g_state.wake_on_touch, on_wake_touch_change
    );

    ui_create_switch_row(
        content, "Wake on Shake", LV_SYMBOL_REFRESH, g_state.wake_on_shake, on_wake_shake_change
    );

    ui_create_switch_row(
        content, "Bateria Grande", LV_SYMBOL_BATTERY_FULL, g_state.big_battery_display, on_big_battery_change
    );

    ui_create_button_row(
        content, "Calibrar Bateria", LV_SYMBOL_SETTINGS, "Calibrar", on_calibrate_battery_click
    );
    ...
    ui_create_switch_row(
        content, "Stealth Boot", LV_SYMBOL_EYE_CLOSE, g_state.stealth_boot, on_stealth_boot_change
    );

    ui_create_button_row(content, "Reiniciar", LV_SYMBOL_REFRESH, "Reboot", on_restart_click);

    ui_create_button_row(content, "Reset Fábrica", LV_SYMBOL_TRASH, "RESET (3x)", on_factory_reset_click);
    ...
    for (int i = -12; i <= 14; i++) {
        ...
        if (i < 14) strcat(tz_options, "\n");
    }
    ui_create_dropdown_row(
        content, "Fuso Horário", LV_SYMBOL_BELL, tz_options, g_state.timezone_offset + 12, on_timezone_change
    );
    ...
    char fw_info[64];
    snprintf(fw_info, sizeof(fw_info), "v%s (%s)", app_desc->version, app_desc->date);
    lbl_firmware = ui_create_info_row(content, "Firmware", LV_SYMBOL_DOWNLOAD, fw_info);
    ...
    lbl_storage = ui_create_info_row(content, "Storage", LV_SYMBOL_DRIVE, storage_info);

    // Temperatura
    char temp_info[32];
    float temp = temperatureRead();
    snprintf(temp_info, sizeof(temp_info), "%.1f°C", temp);
    lbl_temp = ui_create_info_row(content, "Temp. Chip", LV_SYMBOL_CHARGE, temp_info);
}
