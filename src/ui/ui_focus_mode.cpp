...
FocusMode::FocusMode()
    : _screen(nullptr), _clockLabel(nullptr), _mascotLabel(nullptr), _batteryLabel(nullptr),
...
void FocusMode::begin() {
    ...
    _originalBrightness = currentScreenBrightness;
}
...
void FocusMode::setAttackStatus(const char *status, bool active) {
    if (_attackLabel) {
        ...
        lv_obj_set_style_text_color(
            _attackLabel, active ? lv_color_hex(0xFF4444) : lv_color_hex(0x666666), 0
        );
    }
    if (_attackIndicator) {
        if (active) {
...
void FocusMode::applyDimming() {
    if (_config.autoDimScreen) { sys_hw.setDisplayBrightness(_config.dimBrightness * 255 / 100); }
}

void FocusMode::restoreBrightness() { sys_hw.setDisplayBrightness(_originalBrightness * 255 / 100); }

// ═══════════════════════════════════════════════════════════════════════════
// MINIMAL MODE (Stealth Extremo)
// ═══════════════════════════════════════════════════════════════════════════
...
void MinimalMode::disable() {
    if (!_enabled) return;
    _enabled = false;
    sys_hw.setDisplayBrightness(100);
}

void MinimalMode::setMessage(const char *msg) {
    if (_messageLabel) { lv_label_set_text(_messageLabel, msg); }
}

void MinimalMode::showActivity(bool active) {
    if (_activityDot) {
        lv_obj_set_style_bg_color(_activityDot, active ? lv_color_hex(0xFF0000) : lv_color_hex(0x330000), 0);
    }
}
