...
lv_obj_t *ui_mascot_select_create() {
  ...
  // Create mascot cards
  // NOTA: Por enquanto, apenas Lele (ID 0) está ativo
  // Os outros mascotes aparecem como "Em breve"
  for (uint8_t i = 0; i < MascotManager::getMascotCount(); i++) {
    const MascotInfo &info = mascot_manager.getMascotInfo((MascotType)i);
    
    // Verifica se é Lele (único disponível por enquanto)
    bool isAvailable = (i == 0); // Apenas Lele está disponível
    ...
    lv_obj_align(emoji, LV_ALIGN_TOP_MID, 0, 2);
    
    // Se não está disponível, adiciona overlay de cadeado
    if (!isAvailable) {
      lv_obj_set_style_text_opa(emoji, LV_OPA_40, 0);
      
      lv_obj_t *lock = lv_label_create(card);
      lv_label_set_text(lock, "🔒");
      lv_obj_set_style_text_font(lock, &lv_font_montserrat_10, 0);
      lv_obj_align(lock, LV_ALIGN_BOTTOM_MID, 0, -2);
    }
  }
