...
static void on_name_change(lv_event_t *e) {
    ...
    LOG_UI("Dragon Name: %s", g_state.dragon_name);
}
...
static void on_personality_change(uint16_t selected) {
    ...
    const char *personality_names[] = {"Aggro", "Chill", "Psycho", "Cute", "Hacker", "Sleepy"};
    LOG_UI("Dragon Personality: %s", personality_names[selected]);

    // Som especial para cada personalidade
    audioDriver.playSound(SOUND_SUCCESS);
}

static void on_mascot_type_change(uint16_t selected) {
    g_state.mascot_type = selected;
    config_manager.saveDragonSettings();

    const char *typeName = "Dragon";
    if (selected == 1) typeName = "Lele";
    else if (selected == 2) typeName = "Lisa";

    LOG_UI("Mascot Type: %s", typeName);

    // Update avatar immediately if active
    // Optional: Reset name or apply defaults
}

static void on_talk_enabled_change(bool checked) {
    g_state.dragon_talk_enabled = checked;
    config_manager.saveDragonSettings();
    LOG_UI("Dragon Talk: %s", checked ? "ON" : "OFF");
}

static void on_talk_lang_change(uint16_t selected) {
    g_state.dragon_talk_lang = selected;
    config_manager.saveDragonSettings();
    LOG_UI("Dragon Language: %s", selected == 0 ? "PT-BR" : "EN");
}
...
static void on_talk_freq_change(uint16_t selected) {
    ...
    LOG_UI("Dragon Talk Freq: %s", freq_names[selected]);
}
...
static void on_exaggerated_change(bool checked) {
    ...
    config_manager.saveDragonSettings();
    LOG_UI("Dragon Exaggerated: %s", checked ? "ON" : "OFF");
}

static void on_pet_only_change(bool checked) {
    g_state.pet_only_mode = checked;
    config_manager.saveDragonSettings();
    LOG_UI("Pet Only Mode: %s", checked ? "ON" : "OFF");
...
static void create_screen() {
    lv_obj_t *content = ui_create_settings_screen(&scr_dragon, "🐉 Dragão", on_back_click);

    // === NOME DO DRAGÃO ===
    ui_create_section_header(content, "Identidade");

    // Selection: Mascot Type
    ...
    ui_create_switch_row(
        content, "Falas Ativas", LV_SYMBOL_AUDIO, g_state.dragon_talk_enabled, on_talk_enabled_change
    );

    ui_create_dropdown_row(
        content, "Idioma", LV_SYMBOL_LIST, "PT-BR\nEnglish", g_state.dragon_talk_lang, on_talk_lang_change
    );
    ...
    ui_create_switch_row(
        content, "Reações Exageradas", LV_SYMBOL_OK, g_state.dragon_exaggerated, on_exaggerated_change
    );

    ui_create_switch_row(
        content, "Modo Pet Only", LV_SYMBOL_IMAGE, g_state.pet_only_mode, on_pet_only_change
    );
