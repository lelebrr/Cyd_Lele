...
static void create_screen() {
    lv_obj_t *content =
        ui_create_settings_screen(&scr_attack, LV_SYMBOL_WARNING " Ataques Auto", on_back_click);

    // === SEÇÃO WPS ===
    ui_create_section_header(content, "WPS Attacks");

    ui_create_switch_row(
        content, "WPS Pixie Dust", LV_SYMBOL_CHARGE, g_state.wps_pixie_auto, on_wps_pixie_change
    );

    ui_create_switch_row(
        content, "WPS Brute-Force", LV_SYMBOL_LOOP, g_state.wps_bruteforce, on_wps_brute_change
    );

    ui_create_slider_row(
        content, "WPS Flood DoS", LV_SYMBOL_WARNING, 1, 10, g_state.wps_flood_intensity, on_wps_flood_change
    );
    ...
    ui_create_slider_row(
        content, "Rogue APs", LV_SYMBOL_WIFI, 1, 50, g_state.rogue_ap_count, on_rogue_ap_change
    );
    ...
    ui_create_switch_row(
        ...
        "Auto-Capture Novas",
        LV_SYMBOL_DOWNLOAD,
        g_state.auto_capture_new_only,
        on_auto_capture_change
    );

    ui_create_switch_row(
        content, "Auto-Save PCAP", LV_SYMBOL_SAVE, g_state.auto_save_pcap, on_auto_pcap_change
    );

    ui_create_switch_row(
        content, "Attack Favoritas", LV_SYMBOL_OK, g_state.auto_attack_favorites, on_auto_attack_fav_change
    );
    ...
    ui_create_switch_row(
        content, "🔥 INSANE MODE", LV_SYMBOL_WARNING, g_state.insane_mode_enabled, on_insane_mode_change
    );
    ...
    ui_create_switch_row(
        content, "Portal Auto", LV_SYMBOL_KEYBOARD, g_state.captive_portal_auto, on_captive_portal_change
    );
    ...
    ui_create_dropdown_row(
        ...
        "Login Genérico\nGoogle\nFacebook\nWiFi "
        "Grátis\nHotel\nAeroporto\nEmpresa\nBanco\nNetflix\nCustom",
        g_state.captive_template_id,
        on_captive_template_change
    );

    // === SEÇÃO BADUSB ===
    ui_create_section_header(content, "BadUSB");

    ui_create_switch_row(content, "BadUSB Auto", LV_SYMBOL_USB, g_state.badusb_auto, on_badusb_change);
    ...
    ui_create_dropdown_row(
        ...
        "RevShell Win\nRevShell Linux\nWiFi Stealer\nExfil Docs\nCrypto "
        "Miner\nKeylogger\nBackdoor\nInfo Gather\nPrank\nCustom",
        g_state.usb_payload_id,
        on_usb_payload_change
    );
}
