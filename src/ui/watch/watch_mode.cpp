...
void WatchMode::exit() {
    ...
    LOG_UI("Exited watch mode");
}

void WatchMode::update() {
    if (!_active || !_screen) return;

    uint32_t now = millis();
    if (now - _lastUpdate < 1000) return; // Update every second
    _lastUpdate = now;

    // Render based on current watchface
    switch ((WatchfaceType)_config.watchface_id) {
        case WATCHFACE_DIGITAL: renderDigital(); break;
        case WATCHFACE_ANALOG: renderAnalog(); break;
        case WATCHFACE_CYBER: renderCyber(); break;
        case WATCHFACE_MASCOT: renderMascot(); break;
        case WATCHFACE_HACKER: renderHacker(); break;
        case WATCHFACE_MINIMAL: renderMinimal(); break;
        default: renderDigital(); break;
    }
}

void WatchMode::setWatchface(WatchfaceType type) {
    if (type >= WATCHFACE_COUNT) return;
...
void WatchMode::renderDigital() {
    ...
    if (_config.show_handshake_count) {
        ...
        sprintf(stats_str, "🤝 %lu  📡 %lu", g_state.handshakes_captured, g_state.networks_seen);
...
void WatchMode::renderCyber() {
    ...
    sprintf(stats_str, "NET:%lu HSK:%lu DEA:%lu", g_state.networks_seen, g_state.handshakes_captured, 0UL);
    lv_label_set_text(_statsLabel, stats_str);
}
...
void WatchMode::renderMascot() {
    ...
    sprintf(time_str, "%s  %02d:%02d", mascot_manager.getCurrentMascotInfo().emoji, h, m);
    lv_label_set_text(_timeLabel, time_str);
...
void WatchMode::renderHacker() {
    ...
    sprintf(stats_str, "$ captured: %lu | scanned: %lu", g_state.handshakes_captured, g_state.networks_seen);
    lv_label_set_text(_statsLabel, stats_str);
