...
static void on_tx_power_change(int32_t value) {
    ...
    LOG_UI("TX Power: %ld dBm", value);
}

static void on_channel_change(uint16_t selected) {
    g_state.wifi_channel = selected == 0 ? -1 : selected;
    config_manager.saveNetworkSettings();
    LOG_UI("WiFi Channel: %d", g_state.wifi_channel);
}

static void on_randomize_mac() {
    // Gera MAC aleatório
    for (int i = 0; i < 6; i++) { g_state.custom_mac[i] = random(256); }
    g_state.custom_mac[0] = (g_state.custom_mac[0] & 0xFE) | 0x02; // Local unicast
    ...
    if (lbl_mac) {
        ...
        snprintf(
            ...
            g_state.custom_mac[1],
            g_state.custom_mac[2],
            g_state.custom_mac[3],
            g_state.custom_mac[4],
            g_state.custom_mac[5]
        );
        ui_update_info_row(lbl_mac, mac_str);
    }

    LOG_UI(
        "MAC Randomizado: %02X:%02X:%02X:%02X:%02X:%02X",
        g_state.custom_mac[0],
        g_state.custom_mac[1],
        g_state.custom_mac[2],
        g_state.custom_mac[3],
...
static void create_screen() {
    lv_obj_t *content =
        ui_create_settings_screen(&scr_network, LV_SYMBOL_WIFI " Rede & Wireless", on_back_click);
    ...
    ui_create_slider_row(
        content, "Potência TX", LV_SYMBOL_WIFI, 8, 20, g_state.wifi_tx_power_dbm, on_tx_power_change
    );
    ...
    // Randomizar MAC
    ui_create_button_row(content, "Randomizar MAC", LV_SYMBOL_SHUFFLE, "Random", on_randomize_mac);

    // MAC Spoof on Boot
    ui_create_switch_row(
        content, "Spoof MAC Boot", LV_SYMBOL_REFRESH, g_state.mac_spoof_on_boot, on_mac_spoof_boot_change
    );

    // 2.4 GHz
    ui_create_switch_row(
        content, "Rádio 2.4 GHz", LV_SYMBOL_WIFI, g_state.radio_2ghz_enabled, on_2ghz_change
    );
    ...
    // Beacon Spam
    ui_create_switch_row(
        content, "Beacon Spam", LV_SYMBOL_LOOP, g_state.beacon_spam_enabled, on_beacon_spam_change
    );
    ...
    ui_create_slider_row(
        ...
        1,
        10,
        g_state.beacon_spam_speed,
        on_beacon_speed_change
    );

    // Evil Twin
    ui_create_switch_row(
        content, "Evil Twin Auto", LV_SYMBOL_WARNING, g_state.evil_twin_auto, on_evil_twin_change
    );

    // Karma/Mana
    ui_create_switch_row(
        content, "Karma + Mana", LV_SYMBOL_CALL, g_state.karma_mana_enabled, on_karma_mana_change
    );

    // Deauth All
    ui_create_switch_row(
        content, "Deauth Todas", LV_SYMBOL_CLOSE, g_state.deauth_all_networks, on_deauth_all_change
    );

    // Monitor Only
    ui_create_switch_row(
        content, "Modo Monitor", LV_SYMBOL_EYE_OPEN, g_state.monitor_mode_only, on_monitor_only_change
    );

    // Promiscuous
    ui_create_switch_row(
        content, "Promiscuous", LV_SYMBOL_EYE_OPEN, g_state.promiscuous_mode, on_promiscuous_change
    );
    ...
    // BLE Spam
    ui_create_switch_row(
        content, "BLE Spam", LV_SYMBOL_BLUETOOTH, g_state.ble_spam_enabled, on_ble_spam_change
    );
