...
bool setupSdCard() {
#ifndef USE_SD_MMC
    if (leleConfigPins.SDCARD_bus.sck < 0) {
        sdcardMounted = false;
        return false;
    }
#endif
    ...
    if (task) {
        if (!SD.begin((int8_t)leleConfigPins.SDCARD_bus.cs)) result = false;
        // Serial.println("Task not activated");
    }
    // SDCard in the same Bus as TFT, in this case we call the SPI TFT Instance
    else if (leleConfigPins.SDCARD_bus.mosi == (gpio_num_t)TFT_MOSI &&
             leleConfigPins.SDCARD_bus.mosi != GPIO_NUM_NC) {
        LOG_INFO("SDCard in the same Bus as TFT, using TFT SPI instance");
#if TFT_MOSI > 0 // condition for Headless and 8bit displays (no SPI bus)
        if (!SD.begin(leleConfigPins.SDCARD_bus.cs, tft.getSPIinstance())) {
            result = false;
            LOG_ERROR("SDCard in the same Bus as TFT, but failed to mount");
        }
#else
    ...
    NEXT:
        sdcardSPI.begin(
            (int8_t)leleConfigPins.SDCARD_bus.sck,
            (int8_t)leleConfigPins.SDCARD_bus.miso,
            (int8_t)leleConfigPins.SDCARD_bus.mosi,
            (int8_t)leleConfigPins.SDCARD_bus.cs
        ); // start SPI communications
        delay(10);
        if (!SD.begin((int8_t)leleConfigPins.SDCARD_bus.cs, sdcardSPI)) result = false;
        LOG_INFO("SDCard in a different Bus, using sdcardSPI instance");
    }
#endif

    if (result == false) {
        LOG_ERROR("SDCARD NOT mounted, check wiring and format");
        sdcardMounted = false;
        return false;
    } else {
        LOG_INFO("SDCARD mounted successfully");
        sdcardMounted = true;
        return true;
    }
}
...
bool deleteFromSd(FS fs, String path) {
    ...
    LOG_DEBUG("Deleting: %s", path.c_str());
    if (!dir.isDirectory()) {
        dir.close();
...
bool copyToFs(FS from, FS to, String path, bool draw) {
    ...
    if (!LittleFS.begin()) {
        LOG_ERROR("LittleFS not mounted");
        return false;
    }

    File source = from.open(path, FILE_READ);
    if (!source) {
        LOG_ERROR("Fail opening Source file");
        return false;
    }
    ...
    if (!dest) {
        LOG_ERROR("Fail creating destination file");
        return false;
    }
    size_t bytesRead;
    int tot = source.size();
    ...
    uint8_t buff[1024] = {0};
    // tft.drawRect(5,tftHeight-12, (tftWidth-10), 9, leleConfig.priColor);
    while ((bytesRead = source.read(buff, bufSize)) > 0) {
        if (dest.write(buff, bytesRead) != bytesRead) {
            ...
            source.close();
            dest.close();
            LOG_ERROR("Error 5: Write failed");
            return false;
        } else {
            prog += bytesRead;
...
bool pasteFile(FS fs, String path) {
    ...
    while ((bytesRead = sourceFile.read(buff, bufSize)) > 0) {
        if (destFile.write(buff, bytesRead) != bytesRead) {
            ...
            // tft.fillRect(7,tftHeight-10, (tftWidth-14)*prog/tot, 5, leleConfig.priColor);
        }
    }
...
char *readBigFile(FS &fs, String filepath, bool binary, size_t *fileSize) {
    ...
    if (!buf) {
        LOG_ERROR("Could not allocate memory for file: %s", filepath.c_str());
        return NULL;
    }
...
void readFs(FS fs, String folder, String allowed_ext) {
    ...
    LOG_INFO("Files listed with: %d files/folders found", fileList.size());
...
String loopSD(FS &fs, bool filePicker, String allowed_ext, String rootPath) {
    ...
    if (!fs.exists(rootPath)) {
        LOG_WARN("loopSD-> 1st exist test failed");
        rootPath = "/";
        if (!fs.exists(rootPath)) {
            LOG_WARN("loopSD-> 2nd exist test failed");
    ...
    tft.drawPixel(0, 0, 0);
    tft.fillScreen(leleConfig.bgColor); // TODO: Does only the T-Embed CC1101 need this?
    tft.drawRoundRect(5, 5, tftWidth - 10, tftHeight - 10, 5, leleConfig.priColor);
    if (&fs == &SD) {
        if (!setupSdCard()) {
    ...
    while (1) {
        ...
        if (redraw) {
            if (strcmp(PreFolder.c_str(), Folder.c_str()) != 0 || reload) {
                ...
                tft.drawRoundRect(5, 5, tftWidth - 10, tftHeight - 10, 5, leleConfig.priColor);
                LOG_DEBUG("reload to read: %s", Folder.c_str());
                readFs(fs, Folder, allowed_ext);
                PreFolder = Folder;
        ...
        if (LongPress || SelPress) {
            ...
            Files:
                if (fileList[index].folder == true && fileList[index].operation == false) {
                    ...
                    // Debug viewer
                    LOG_DEBUG("Folder: %s", Folder.c_str());
                    redraw = true;
                } else if (fileList[index].folder == false && fileList[index].operation == false) {
                    ...
                    // Debug viewer
                    LOG_DEBUG("%s --> %s", filepath.c_str(), filename.c_str());
                    ...
                    options = {
                        ...
                        {"Renomear",   [=]() { renameFile(fs, filepath, filename); }},
                        {"Copiar",     [=]() { copyFile(fs, filepath); }            },
                        {"Excluir",    [=]() { deleteFromSd(fs, filepath); }        },
                        {"New Folder", [=]() { createFolder(fs, Folder); }          },
                    };
                    if (fileToCopy != "") options.push_back({"Colar", [=]() { pasteFile(fs, Folder); }});
                    if (&fs == &SD)
...
/*********************************************************************
**  Function: setupLeleFolders
**  Cria estrutura de pastas Lele no SD Card
**********************************************************************/
bool setupLeleFolders() {
    if (!setupSdCard()) {
        Serial.println("SD Card not mounted - cannot create Lele folders");
        return false;
    }
    ...
    for (const char *folder : folders) {
        if (!SD.exists(folder)) {
            ...
            Serial.printf("Folder exists: %s\n", folder);
        }
    }

    // Cria arquivo README.txt na pasta raiz lele
    if (!SD.exists("/lele/README.txt")) {
        File readme = SD.open("/lele/README.txt", FILE_WRITE);
        if (readme) {
            readme.println("=== Lele Origin ===");
            readme.println("");
            readme.println("Estrutura de pastas:");
            readme.println("  /logs       - Logs de ataques e operacoes");
            readme.println("  /handshakes - Capturas WiFi (PMKID, WPA)");
            readme.println("  /config     - Configuracoes e backups");
            readme.println("  /captures   - Capturas de pacotes");
            readme.println("  /scripts    - Scripts BadUSB e JS");
            readme.println("  /ir         - Arquivos IR");
            readme.println("  /rf         - Arquivos SubGHz");
            readme.close();
            Serial.println("Created /lele/README.txt");
        }
    }
