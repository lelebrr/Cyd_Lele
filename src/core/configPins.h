...
public:
    ...
    const char *filepath = "/LelePins.conf";
    ...
    // Dual NRF24 Support (Radio 2 on HSPI)
#ifdef NRF24_2_SCK_PIN
    SPIPins NRF24_2_bus = {
        (gpio_num_t)NRF24_2_SCK_PIN,
        (gpio_num_t)NRF24_2_MISO_PIN,
        (gpio_num_t)NRF24_2_MOSI_PIN,
        (gpio_num_t)NRF24_2_SS_PIN,
        (gpio_num_t)NRF24_2_CE_PIN
    };
#else
    // Default HSPI pins for CYD 2432S028R
    SPIPins NRF24_2_bus = {
        (gpio_num_t)14, // SCK
        (gpio_num_t)12, // MISO
        (gpio_num_t)13, // MOSI
        (gpio_num_t)22, // CS (P3 Header)
        (gpio_num_t)27  // CE (P3 Header)
    };
#endif
    ...
    void setNrf24Pins(SPIPins value, int radio_id = 0); // 0=Primary, 1=Secondary
