#ifndef __Lele_CONFIG_H__
#define __Lele_CONFIG_H__
...
public:
    ...
    // WS2812B LEDs (externo)
    int ws2812bEnabled = 1;
    int ws2812bBright = 50;
    uint32_t ws2812bColor = 0xFF0000;  // Vermelho padrão
    int ws2812bEffect = 0;
    int ws2812bEffectSpeed = 5;
    int ws2812bNotifyErrors = 1;
    int ws2812bNotifySuccess = 1;
    int ws2812bNotifyCapture = 1;

    // Sensor Temperatura (DS18B20)
    int tempEnabled = 1;
    float tempWarning = 35.0f;
    float tempCritical = 45.0f;
    float tempShutdown = 50.0f;
    int tempAlertType = 2;     // 1=LED, 2=Screen, 4=Sound, 7=All
    int tempAction = 0;        // 0=Warn, 1=Pause, 2=Shutdown
    int tempShowOnBar = 1;

    // Wifi
    Credential webUI = {"admin", "LELE"};
    std::vector<String> webUISessions = {}; // FIFO queue of session tokens
    WiFiCredential wifiAp = {"Lele_AP", "Lele_AP"};
    ...
    std::vector<QrCodeEntry> qrCodes = {
        {"Lele AP",   "WIFI:T:WPA;S:LeleNet;P:Lelenet;;"},
        {"Lele Wiki", "https://github.com/pr3y/Lele/wiki"},
        {"Lele Site", "https://github.com/pr3y/Lele"            },
