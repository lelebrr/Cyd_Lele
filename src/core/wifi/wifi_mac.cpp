...
void applyConfiguredMAC() {
    if (leleConfig.wifiMAC.length() == 17 && validateMACFormat(leleConfig.wifiMAC)) {
        uint8_t newMAC[6];
        sscanf(
            leleConfig.wifiMAC.c_str(),
        ...
        if (esp_wifi_set_mac(WIFI_IF_STA, newMAC) == ESP_OK) {
            LOG_WIFI("Custom MAC applied: %s", leleConfig.wifiMAC.c_str());
        } else {
            LOG_WARN("Failed to apply custom MAC, using default");
        }
    }
}
...
void wifiMACMenu() {
    ...
    if (leleConfig.wifiMAC != "" && validateMACFormat(leleConfig.wifiMAC)) {
        currentMAC = leleConfig.wifiMAC + " (Custom)";
    } else {
