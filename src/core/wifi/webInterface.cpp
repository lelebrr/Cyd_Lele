...
bool checkUserWebAuth(AsyncWebServerRequest *request, bool onFailureReturnLoginPage = false) {
    if (request->hasHeader("Cookie")) {
        ...
        int idx = c.indexOf("LeleSESSION=");
        if (idx != -1) {
            int start = idx + 13;
            int end = c.indexOf(';', start);
            if (end == -1) end = c.length();
            String token = c.substring(start, end);
            if (leleConfig.isValidWebUISession(token)) { return true; }
        }
    }
...
void drawWebUiScreen(bool mode_ap) {
    ...
    if (mode_ap) {
        setTftDisplay(0, 0, leleConfig.bgColor, FM);
        tft.drawCentreString("LeleNet/lelenet", tftWidth / 2, 7, 1);
    }
    setTftDisplay(0, 0, ALCOLOR, FM);
    tft.drawCentreString("Lele WebUI", tftWidth / 2, 27, 1);
    String txt;
    if (!mode_ap) txt = WiFi.localIP().toString();
    else txt = WiFi.softAPIP().toString();
    tft.setTextColor(leleConfig.priColor);

    tft.drawCentreString("http://Lele.local", tftWidth / 2, 45, 1);
    setTftDisplay(7, 67);
    ...
    tft.println("Usr: " + String(leleConfig.webUI.user));
    tft.setCursor(7, tft.getCursorY());
    tft.println("Pwd: " + String(leleConfig.webUI.pwd));
    tft.setCursor(7, tft.getCursorY());
...
void serveWebUIFile(
    ...
    FS *fs = NULL;
    if (setupSdCard()) {
        if (SD.exists("/LeleWebUI/" + filename)) fs = &SD;
        UNMOUNT_SD_CARD;

    } else if (LittleFS.exists("/LeleWebUI/" + filename)) {
        fs = &LittleFS;
    }
    if (fs) {
        response = request->beginResponse(*fs, "/LeleWebUI/" + filename, contentType);
        UNMOUNT_SD_CARD;
    } else {
        if (filename == "theme.css") {
            String css = ":root{--color:" + color565ToWebHex(leleConfig.priColor) +
                         ";--sec-color:" + color565ToWebHex(leleConfig.secColor) +
                         ";--background:" + color565ToWebHex(leleConfig.bgColor) + ";}";
        ...
        if (gzip) {
            if (!response->addHeader("Content-Encoding", "gzip")) LOG_WIFI("Failed to add gzip header");
        }
    }
...
static bool startMdnsResponder() {
    ...
    if (!MDNS.begin(host)) {
        LOG_WIFI("Error setting up MDNS responder!");
        return false;
    }
...
void configureWebServer() {
    ...
    server->on("/login", HTTP_POST, [](AsyncWebServerRequest *request) {
        if (request->hasParam("username", true) && request->hasParam("password", true)) {
            ...
            if (username == leleConfig.webUI.user && password == leleConfig.webUI.pwd) {
                String token = generateToken();
                AsyncWebServerResponse *response = request->beginResponse(302);
                response->addHeader("Location", "/");
                response->addHeader("Set-Cookie", "LeleSESSION=" + token + "; Path=/; HttpOnly");
                request->send(response);
                MOUNT_SD_CARD;
                leleConfig.addWebUISession(token);
                UNMOUNT_SD_CARD;
                return;
            }
        }
    ...
    server->on("/logout", HTTP_GET, [](AsyncWebServerRequest *request) {
        if (request->hasHeader("Cookie")) {
            ...
            if (idx != -1) {
                ...
                if (end == -1) end = c.length();
                String token = c.substring(start, end);
                leleConfig.removeWebUISession(token);
            }
        }
        AsyncWebServerResponse *response = request->beginResponse(302);
        response->addHeader("Location", "/?loggedout");
        response->addHeader("Set-Cookie", "LeleSESSION=0; Path=/; Expires=Thu, 01 Jan 1970 00:00:00 GMT");
        request->send(response);
    });
    ...
    server->on("/file", HTTP_GET, [](AsyncWebServerRequest *request) {
        if (checkUserWebAuth(request)) {
            if (request->hasArg("name") && request->hasArg("action")) {
                ...
                LOG_WIFI("fileAction: %s", fileAction);
                ...
                if (!fs->exists(fileName)) {
                    ...
                    } else if (strcmp(fileAction.c_str(), "Excluir") == 0) {
    ...
    server->on("/wifi", HTTP_GET, [](AsyncWebServerRequest *request) {
        if (checkUserWebAuth(request)) {
            if (request->hasArg("usr") && request->hasArg("pwd")) {
                ...
                MOUNT_SD_CARD;
                leleConfig.setWebUICreds(usr, pwd);
                UNMOUNT_SD_CARD;
                request->send(
...
void startWebUi(bool mode_ap) {
    ...
    if (!server) {
        ...
        LOG_WIFI("Configuring Webserver ...");
