...
void setBrightness(uint8_t brightval, bool save) {
    if (leleConfig.bright > 100) leleConfig.setBright(100);
    _setBrightness(brightval);
    delay(10);

    currentScreenBrightness = brightval;
    if (save) { leleConfig.setBright(brightval); }
}
...
**********************************************************************/
void getBrightness() {
    if (leleConfig.bright > 100) {
        leleConfig.setBright(100);
        _setBrightness(leleConfig.bright);
        delay(10);
        setBrightness(100);
    }
    ...
    currentScreenBrightness = leleConfig.bright;
}
...
int gsetRotation(bool set) {
    int getRot = leleConfig.rotation;
    int result = ROTATION;
...
void setBrightnessMenu() {
    int idx = 0;
    if (leleConfig.bright == 100) idx = 0;
    else if (leleConfig.bright == 75) idx = 1;
    else if (leleConfig.bright == 50) idx = 2;
    else if (leleConfig.bright == 25) idx = 3;
    else if (leleConfig.bright == 1) idx = 4;

    options = {
        {"100%",
         [=]() { setBrightness((uint8_t)100); },
         leleConfig.bright == 100,
...
void setDimmerTimeMenu() {
    int idx = 0;
    if (leleConfig.dimmerSet == 10) idx = 0;
    else if (leleConfig.dimmerSet == 20) idx = 1;
    else if (leleConfig.dimmerSet == 30) idx = 2;
    else if (leleConfig.dimmerSet == 60) idx = 3;
    else if (leleConfig.dimmerSet == 0) idx = 4;
    options = {
        {"10s",      [=]() { leleConfig.setDimmer(10); }, leleConfig.dimmerSet == 10},
        {"20s",      [=]() { leleConfig.setDimmer(20); }, leleConfig.dimmerSet == 20},
        {"30s",      [=]() { leleConfig.setDimmer(30); }, leleConfig.dimmerSet == 30},
        {"60s",      [=]() { leleConfig.setDimmer(60); }, leleConfig.dimmerSet == 60},
        {"Disabled", [=]() { leleConfig.setDimmer(0); },  leleConfig.dimmerSet == 0 },
    };
...
void setUIColor() {
    ...
    while (1) {
        ...
        for (const auto &mapping : UI_COLORS) {
            if (leleConfig.priColor == mapping.priColor && leleConfig.secColor == mapping.secColor &&
                leleConfig.bgColor == mapping.bgColor) {
                idx = i;
            }
            ...
            options.emplace_back(
                ...
                [=, &mapping]() {
                    ...
                    leleConfig.setUiColor(mapping.priColor, &secColor, &bgColor);
                },
                idx == i
            );
            ++i;
        ...
        options.push_back(
            {"Custom Color",
             [=]() {
                 uint16_t oldPriColor = leleConfig.priColor;
                 uint16_t oldSecColor = leleConfig.secColor;
                 uint16_t oldBgColor = leleConfig.bgColor;

                 if (setCustomUIColorMenu()) {
                     leleConfig.setUiColor(leleConfig.priColor, &leleConfig.secColor, &leleConfig.bgColor);
                 } else {
                     leleConfig.priColor = oldPriColor;
                     leleConfig.secColor = oldSecColor;
                     leleConfig.bgColor = oldBgColor;
                 }
        ...
        options.push_back(
            {"Invert Color",
             [=]() {
                 leleConfig.setColorInverted(!leleConfig.colorInverted);
                 tft.invertDisplay(leleConfig.colorInverted);
             },
             leleConfig.colorInverted}
        );
...
void setCustomUIColorSettingMenu(
    ...
    uint16_t color = (colorType == 1)   ? leleConfig.priColor
                     : (colorType == 2) ? leleConfig.secColor
                                        : leleConfig.bgColor;
        ...
        if (i == 0 || (rgb == 2 && (i + 1) % 2 == 0) || (rgb != 2)) {
            ...
            options.emplace_back(
                ...
                [colorType, updatedColor]() {
                    if (colorType == 1) leleConfig.priColor = updatedColor;
                    else if (colorType == 2) leleConfig.secColor = updatedColor;
                    else leleConfig.bgColor = updatedColor;
                },
                selectedIndex == i,
    ...
    if (selectedOption == -1 || selectedOption == options.size() - 1) {
        if (colorType == 1) {
            leleConfig.priColor = color;
        } else if (colorType == 2) {
            leleConfig.secColor = color;
        } else {
            leleConfig.bgColor = color;
        }
        return;
    }
}
...
void setSoundConfig() {
    options = {
        {"Sound off", [=]() { leleConfig.setSoundEnabled(0); }, leleConfig.soundEnabled == 0},
        {"Sound on",  [=]() { leleConfig.setSoundEnabled(1); }, leleConfig.soundEnabled == 1},
    };
    loopOptions(options, leleConfig.soundEnabled);
}
...
void setSoundVolume() {
    options = {
        {"10%",  [=]() { leleConfig.setSoundVolume(10); },  leleConfig.soundVolume == 10 },
        {"20%",  [=]() { leleConfig.setSoundVolume(20); },  leleConfig.soundVolume == 20 },
        {"30%",  [=]() { leleConfig.setSoundVolume(30); },  leleConfig.soundVolume == 30 },
        {"40%",  [=]() { leleConfig.setSoundVolume(40); },  leleConfig.soundVolume == 40 },
        {"50%",  [=]() { leleConfig.setSoundVolume(50); },  leleConfig.soundVolume == 50 },
        {"60%",  [=]() { leleConfig.setSoundVolume(60); },  leleConfig.soundVolume == 60 },
        {"70%",  [=]() { leleConfig.setSoundVolume(70); },  leleConfig.soundVolume == 70 },
        {"80%",  [=]() { leleConfig.setSoundVolume(80); },  leleConfig.soundVolume == 80 },
        {"90%",  [=]() { leleConfig.setSoundVolume(90); },  leleConfig.soundVolume == 90 },
        {"100%", [=]() { leleConfig.setSoundVolume(100); }, leleConfig.soundVolume == 100},
    };
    loopOptions(options, leleConfig.soundVolume);
}
...
void setLedBlinkConfig() {
    options = {
        {"Led Blink off", [=]() { leleConfig.setLedBlinkEnabled(0); }, leleConfig.ledBlinkEnabled == 0},
        {"Led Blink on",  [=]() { leleConfig.setLedBlinkEnabled(1); }, leleConfig.ledBlinkEnabled == 1},
    };
    loopOptions(options, leleConfig.ledBlinkEnabled);
}
#endif
...
void setWifiStartupConfig() {
    options = {
        {"Disable", [=]() { leleConfig.setWifiAtStartup(0); }, leleConfig.wifiAtStartup == 0},
        {"Enable",  [=]() { leleConfig.setWifiAtStartup(1); }, leleConfig.wifiAtStartup == 1},
    };
    loopOptions(options, leleConfig.wifiAtStartup);
}
...
void addEvilWifiMenu() {
    ...
    leleConfig.addEvilWifiName(apName);
}
...
void removeEvilWifiMenu() {
    ...
    for (const auto &wifi_name : leleConfig.evilWifiNames) {
        options.push_back({wifi_name.c_str(), [wifi_name]() { leleConfig.removeEvilWifiName(wifi_name); }});
    }
...
void setEvilEndpointCreds() {
    String userInput = keyboard(leleConfig.evilPortalEndpoints.getCredsEndpoint, 30, "Evil creds endpoint");
    leleConfig.setEvilEndpointCreds(userInput);
}
...
void setEvilEndpointSsid() {
    String userInput = keyboard(leleConfig.evilPortalEndpoints.setSsidEndpoint, 30, "Evil creds endpoint");
    leleConfig.setEvilEndpointSsid(userInput);
}
...
void setEvilAllowGetCreds() {
    options = {
        {"Disallow",
         [=]() { leleConfig.setEvilAllowGetCreds(false); },
         leleConfig.evilPortalEndpoints.allowGetCreds == false},
        {"Allow",
         [=]() { leleConfig.setEvilAllowGetCreds(true); },
         leleConfig.evilPortalEndpoints.allowGetCreds == true },
    };
    loopOptions(options, leleConfig.evilPortalEndpoints.allowGetCreds);
}
...
void setEvilAllowSetSsid() {
    options = {
        {"Disallow",
         [=]() { leleConfig.setEvilAllowSetSsid(false); },
         leleConfig.evilPortalEndpoints.allowSetSsid == false},
        {"Allow",
         [=]() { leleConfig.setEvilAllowSetSsid(true); },
         leleConfig.evilPortalEndpoints.allowSetSsid == true },
    };
    loopOptions(options, leleConfig.evilPortalEndpoints.allowSetSsid);
}
...
void setEvilAllowEndpointDisplay() {
    options = {
        {"Disallow",
         [=]() { leleConfig.setEvilAllowEndpointDisplay(false); },
         leleConfig.evilPortalEndpoints.showEndpoints == false},
        {"Allow",
         [=]() { leleConfig.setEvilAllowEndpointDisplay(true); },
         leleConfig.evilPortalEndpoints.showEndpoints == true },
    };
    loopOptions(options, leleConfig.evilPortalEndpoints.showEndpoints);
}
...
void setEvilPasswordMode() {
    options = {
        {"Save 'password'",
         [=]() { leleConfig.setEvilPasswordMode(FULL_PASSWORD); },
         leleConfig.evilPortalPasswordMode == FULL_PASSWORD  },
        {"Save 'p******d'",
         [=]() { leleConfig.setEvilPasswordMode(FIRST_LAST_CHAR); },
         leleConfig.evilPortalPasswordMode == FIRST_LAST_CHAR},
        {"Save '*hidden*'",
         [=]() { leleConfig.setEvilPasswordMode(HIDE_PASSWORD); },
         leleConfig.evilPortalPasswordMode == HIDE_PASSWORD  },
        {"Save length",
         [=]() { leleConfig.setEvilPasswordMode(SAVE_LENGTH); },
         leleConfig.evilPortalPasswordMode == SAVE_LENGTH    },
    };
    loopOptions(options, leleConfig.evilPortalPasswordMode);
}
...
void setRFModuleMenu() {
    ...
    if (leleConfig.rfModule == M5_RF_MODULE) idx = 0;
    else if (leleConfig.rfModule == CC1101_SPI_MODULE) {
        idx = 1;
#if defined(ARDUINO_M5STICK_C_PLUS) || defined(ARDUINO_M5STICK_C_PLUS2)
        if (leleConfigPins.CC1101_bus.mosi == GPIO_NUM_26) idx = 2;
#endif
    }
    ...
    if (result == CC1101_SPI_MODULE || pins_setup > 0) {
        ...
        if (initRfModule()) {
            leleConfig.setRfModule(CC1101_SPI_MODULE);
            return;
        }
        // else display an error
        displayError("CC1101 not found", true);
        if (pins_setup == 1)
            qrcode_display("https://github.com/pr3y/Lele/blob/main/media/connections/cc1101_stick.jpg");
        if (pins_setup == 2)
            qrcode_display(
                "https://github.com/pr3y/Lele/blob/main/media/connections/cc1101_stick_SDCard.jpg"
            );
        while (!check(AnyKeyPress)) vTaskDelay(50 / portTICK_PERIOD_MS);
    }
    // fallback to "M5 RF433T/R" on errors
    leleConfig.setRfModule(M5_RF_MODULE);
}
...
void setRFFreqMenu() {
    float result = 433.92;
    String freq_str = keyboard(String(leleConfig.rfFreq), 10, "Default frequency:");
    if (freq_str.length() > 1) {
        ...
        if (result >= 280 && result <= 928) { // TODO: check valid freq according to current module?
            leleConfig.setRfFreq(result);
            return;
        }
    }
    // else
    displayError("Invalid frequency");
    leleConfig.setRfFreq(433.92); // reset to default
    delay(1000);
}
...
void setRFIDModuleMenu() {
    options = {
        {"M5 RFID2",
         [=]() { leleConfig.setRfidModule(M5_RFID2_MODULE); },
         leleConfig.rfidModule == M5_RFID2_MODULE     },
#ifdef M5STICK
        {"PN532 I2C G33",
         [=]() { leleConfig.setRfidModule(PN532_I2C_MODULE); },
         leleConfig.rfidModule == PN532_I2C_MODULE    },
        {"PN532 I2C G36",
         [=]() { leleConfig.setRfidModule(PN532_I2C_SPI_MODULE); },
         leleConfig.rfidModule == PN532_I2C_SPI_MODULE},
#else
        {"PN532 on I2C",
         [=]() { leleConfig.setRfidModule(PN532_I2C_MODULE); },
         leleConfig.rfidModule == PN532_I2C_MODULE},
#endif
        {"PN532 on SPI",
         [=]() { leleConfig.setRfidModule(PN532_SPI_MODULE); },
         leleConfig.rfidModule == PN532_SPI_MODULE    },
        {"RC522 on SPI",
         [=]() { leleConfig.setRfidModule(RC522_SPI_MODULE); },
         leleConfig.rfidModule == RC522_SPI_MODULE    },
    };
    loopOptions(options, leleConfig.rfidModule);
}
...
void setClock() {
    ...
    if (auto_mode) {
        ...
        float selectedTimezone = leleConfig.tmz; // Store current timezone as default
        ...
        for (const auto &mapping : timezoneMappings) {
            if (leleConfig.tmz == mapping.offset) { idx = i; }

            options.emplace_back(
                mapping.name, [=, &mapping]() { leleConfig.setTmz(mapping.offset); }, idx == i
            );
            ++i;
        }
        ...
        timeClient.setTimeOffset(leleConfig.tmz * 3600);
        timeClient.begin();
        timeClient.update();
...
#endif
            LOG_DEBUG("Current time: %s", timeStr.c_str());
            tft.setTextColor(leleConfig.priColor, leleConfig.bgColor);
            tft.drawRect(
                BORDER_PAD_X,
                BORDER_PAD_X,
...
int gsetIrTxPin(bool set) {
    int result = leleConfig.irTx;

    if (result > 50) leleConfig.setIrTxPin(LED);
    if (set) {
        options.clear();
        ...
        int j = 0;
        for (auto pin : pins) {
            if (pin.second == leleConfig.irTx && idx == 100) idx = j;
            j++;
...
#endif
                options.push_back(
                    {pin.first, [=]() { leleConfig.setIrTxPin(pin.second); }, pin.second == leleConfig.irTx}
                );
        }
        ...
        LOG_DEBUG("Saved pin: %d", leleConfig.irTx);
    }

    returnToMenu = true;
    return leleConfig.irTx;
}
...
void setIrTxRepeats() {
    ...
    options = {
        ...
        {"Custom",           [&]() {
             // up to 99 repeats
             String rpt = keyboard(String(leleConfig.irTxRepeats), 2, "Nbr of Repeats (+ 1 initial)");
    ...
    leleConfig.setIrTxRepeats(chRpts);
}
/*********************************************************************
...
int gsetIrRxPin(bool set) {
    int result = leleConfig.irRx;

    if (result > 45) leleConfig.setIrRxPin(GROVE_SCL);
    if (set) {
        options.clear();
        std::vector<std::pair<const char *, int>> pins;
        pins = IR_RX_PINS;
        ...
        int j = 0;
        for (auto pin : pins) {
            if (pin.second == leleConfig.irRx && idx < 0) idx = j;
            j++;
#ifdef ALLOW_ALL_GPIO_FOR_IR_RF
            int i = pin.second;
            if (i != TFT_CS && i != TFT_RST && i != TFT_SCLK && i != TFT_MOSI && i != TFT_BL &&
...
#endif
                options.push_back(
                    {pin.first, [=]() { leleConfig.setIrRxPin(pin.second); }, pin.second == leleConfig.irRx}
                );
        }
...
int gsetRfTxPin(bool set) {
    int result = leleConfig.rfTx;

    if (result > 45) leleConfig.setRfTxPin(GROVE_SDA);
    if (set) {
        options.clear();
        std::vector<std::pair<const char *, int>> pins;
        pins = RF_TX_PINS;
        ...
        int j = 0;
        for (auto pin : pins) {
            if (pin.second == leleConfig.rfTx && idx < 0) idx = j;
            j++;
#ifdef ALLOW_ALL_GPIO_FOR_IR_RF
            int i = pin.second;
            if (i != TFT_CS && i != TFT_RST && i != TFT_SCLK && i != TFT_MOSI && i != TFT_BL &&
...
#endif
                options.push_back(
                    {pin.first, [=]() { leleConfig.setRfTxPin(pin.second); }, pin.second == leleConfig.rfTx}
                );
        }
...
int gsetRfRxPin(bool set) {
    int result = leleConfig.rfRx;

    if (result > 36) leleConfig.setRfRxPin(GROVE_SCL);
    if (set) {
        options.clear();
        std::vector<std::pair<const char *, int>> pins;
        pins = RF_RX_PINS;
        ...
        int j = 0;
        for (auto pin : pins) {
            if (pin.second == leleConfig.rfRx && idx < 0) idx = j;
            j++;
#ifdef ALLOW_ALL_GPIO_FOR_IR_RF
            int i = pin.second;
            if (i != TFT_CS && i != TFT_RST && i != TFT_SCLK && i != TFT_MOSI && i != TFT_BL &&
...
#endif
                options.push_back(
                    {pin.first, [=]() { leleConfig.setRfRxPin(pin.second); }, pin.second == leleConfig.rfRx}
                );
        }
...
void setStartupApp() {
    int idx = 0;
    if (leleConfig.startupApp == "") idx = 0;

    options = {
        {"None", [=]() { leleConfig.setStartupApp(""); }, leleConfig.startupApp == ""}
    };

    int index = 1;
    for (String appName : startupApp.getAppNames()) {
        if (leleConfig.startupApp == appName) idx = index++;

        options.push_back(
            {appName.c_str(), [=]() { leleConfig.setStartupApp(appName); }, leleConfig.startupApp == appName}
        );
    }
...
void setGpsBaudrateMenu() {
    options = {
        {"9600 bps",   [=]() { leleConfig.setGpsBaudrate(9600); },   leleConfig.gpsBaudrate == 9600  },
        {"19200 bps",  [=]() { leleConfig.setGpsBaudrate(19200); },  leleConfig.gpsBaudrate == 19200 },
        {"38400 bps",  [=]() { leleConfig.setGpsBaudrate(38400); },  leleConfig.gpsBaudrate == 38400 },
        {"57600 bps",  [=]() { leleConfig.setGpsBaudrate(57600); },  leleConfig.gpsBaudrate == 57600 },
        {"115200 bps", [=]() { leleConfig.setGpsBaudrate(115200); }, leleConfig.gpsBaudrate == 115200},
    };

    loopOptions(options, leleConfig.gpsBaudrate);
}
...
void setBleNameMenu() {
    ...
    const bool isDefault = leleConfig.bleName == defaultBleName;

    options = {
        {"Default", [=]() { leleConfig.setBleName(defaultBleName); }, isDefault },
        {"Custom",
         [=]() {
             String newBleName = keyboard(leleConfig.bleName, 30, "BLE Device Name:");
             if (!newBleName.isEmpty()) leleConfig.setBleName(newBleName);
...
void setWifiApSsidMenu() {
    const bool isDefault = leleConfig.wifiAp.ssid == "Lele_AP";

    options = {
        {"Default (LeleNet)",
         [=]() { leleConfig.setWifiApCreds("LeleNet", leleConfig.wifiAp.pwd); },
         isDefault                                                                         },
        {"Custom",
         [=]() {
             String newSsid = keyboard(leleConfig.wifiAp.ssid, 32, "WiFi AP SSID:");
             if (!newSsid.isEmpty()) leleConfig.setWifiApCreds(newSsid, leleConfig.wifiAp.pwd);
...
void setWifiApPasswordMenu() {
    const bool isDefault = leleConfig.wifiAp.pwd == "LeleNet";

    options = {
        {"Default (LeleNet)",
         [=]() { leleConfig.setWifiApCreds(leleConfig.wifiAp.ssid, "LeleNet"); },
         isDefault                                                                          },
        {"Custom",
         [=]() {
             String newPassword = keyboard(leleConfig.wifiAp.pwd, 32, "WiFi AP Password:");
             if (!newPassword.isEmpty()) leleConfig.setWifiApCreds(leleConfig.wifiAp.ssid, newPassword);
...
void setBadUSBBLEKeyboardLayoutMenu() {
    ...
    uint8_t opt = leleConfig.badUSBBLEKeyboardLayout;
    ...
    if (opt != leleConfig.badUSBBLEKeyboardLayout) { leleConfig.setBadUSBBLEKeyboardLayout(opt); }
}
...
void setBadUSBBLEKeyDelayMenu() {
    String delayStr = keyboard(String(leleConfig.badUSBBLEKeyDelay), 4, "Key Delay (ms):");
    uint8_t delayVal = static_cast<uint8_t>(delayStr.toInt());
    if (delayVal >= 25 && delayVal <= 500) {
        leleConfig.setBadUSBBLEKeyDelay(delayVal);
    } else if (delayVal != 0) {
...
void setMacAddressMenu() {
    ...
    String currentMAC = leleConfig.wifiMAC;
    if (currentMAC == "") currentMAC = WiFi.macAddress();

    options.clear();
    options = {
        {"Default MAC (" + WiFi.macAddress() + ")",
         [&]() { leleConfig.setWifiMAC(""); },
         leleConfig.wifiMAC == ""},
        {"Set Custom MAC",
         [&]() {
             String newMAC = keyboard(leleConfig.wifiMAC, 17, "XX:YY:ZZ:AA:BB:CC");
             if (newMAC.length() == 17) {
                 leleConfig.setWifiMAC(newMAC);
             } else {
        ...
        {"Random MAC", [&]() {
             ...
             leleConfig.setWifiMAC(String(buf));
         }}
    };
...
RELOAD:
    ...
    if (opt == 0) return;
        ...
        if (changed) {
            value = points;
            leleConfigPins.setSpiPins(value);
        }
    } else {
        options = {};
...
RELOAD:
    ...
    if (opt == 0) return;
        ...
        if (changed) {
            value = points;
            leleConfigPins.setUARTPins(value);
        }
    } else {
        options = {};
        gpio_num_t sel = GPIO_NUM_NC;
...
RELOAD:
    ...
    if (opt == 0) return;
        ...
        if (changed) {
            value = points;
            leleConfigPins.setI2CPins(value);
        }
    } else {
        options = {};
        gpio_num_t sel = GPIO_NUM_NC;
...
void setTheme() {
    ...
    options = {
        ...
        {"Default",
         [&]() {
             leleConfig.removeTheme();
             leleConfig.themePath = "";
             leleConfig.theme.fs = 0;
             leleConfig.secColor = DEFAULT_SECCOLOR;
             leleConfig.bgColor = TFT_BLACK;
             leleConfig.setUiColor(DEFAULT_PRICOLOR);
#ifdef HAS_RGB_LED
             leleConfig.ledBright = 50;
             leleConfig.ledColor = 0x960064;
             leleConfig.ledEffect = 0;
             leleConfig.ledEffectSpeed = 5;
             leleConfig.ledEffectDirection = 1;
             ledSetup();
#endif
             leleConfig.saveFile();
             fs = nullptr;
    ...
    if (leleConfig.openThemeFile(fs, filepath, true)) {
        leleConfig.themePath = filepath;
        if (fs == &LittleFS) leleConfig.theme.fs = 1;
        else if (fs == &SD) leleConfig.theme.fs = 2;
        else leleConfig.theme.fs = 0;
