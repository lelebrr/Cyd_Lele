...
/**
 * @file clients.cpp
 * @brief Clientes SSH e Telnet com display otimizado
 * @note Display implementado com scroll automático e buffer de linha para evitar glitches
 */
...
// Helper para scroll suave do display
static void scrollDisplayIfNeeded() {
    if (tft.getCursorY() > tftHeight - 20) {
        // Scroll suave: move conteúdo para cima
        tft.fillScreen(leleConfig.bgColor);
        tft.setCursor(0, 0);
    }
}

// Helper para imprimir com scroll automático
static void printWithScroll(const char* text) {
    for (int i = 0; text[i] != '\0'; i++) {
        if (text[i] == '\r') continue;
        if (text[i] == '\n' || tft.getCursorX() > tftWidth - 10) {
            tft.println();
            scrollDisplayIfNeeded();
        }
        if (text[i] != '\n') {
            tft.print(text[i]);
        }
            ...
            if (currentMillis - lastKeyPressMillis >= debounceDelay) {
                ...
                if (key.del && commandBuffer.length() > 2) {
                    ...
                    if (cursorY > tftHeight) {
                        ...
                        tft.fillRect(0, tftHeight - 11, tftWidth, 11, leleConfig.bgColor);
                    }
                }
            }
        }
