...
void responder() {
  ...
  tft.fillScreen(leleConfig.bgColor);
  //M5Cardputer.Display.setTextColor(leleConfig.priColor, leleConfig.bgColor);
  if (!wifiConnected) wifiConnectMenu();

  netbiosname_str = keyboard("LELE", 20);
  netbiosName = stringTochar(netbiosname_str);
  netbiosdomain_str = keyboard("LELEGROUP", 20);
  netbiosDomain = stringTochar(netbiosdomain_str);
  dnsdomain_str = keyboard("Lele.Local", 20);
