...
void EMVReader::save_emv(const char *aid, const char *pan, const char *validfrom, const char *validto) {
    ...
    if (!(*fs).exists("/LeleRFID")) (*fs).mkdir("/LeleRFID");
    if (!(*fs).exists("/LeleRFID/Scans")) (*fs).mkdir("/LeleRFID/Scans");
    ...
    File file = (*fs).open("/LeleRFID/Scans/" + filename, FILE_WRITE);
