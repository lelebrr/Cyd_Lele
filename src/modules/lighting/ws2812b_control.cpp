...
void updateWS2812B() {
    ...
    switch (currentEffect) {
        ...
        case EFFECT_BREATHE: {
            ...
            if (breatheVal >= 255) { breatheVal = 255; breatheDir = -5; }
            if (breatheVal <= 0) { breatheVal = 0; breatheDir = 5; }
...
void setLedColor(uint8_t led, RGBColor color) {
    setLedColor(led, color.r, color.g, color.b);
}
...
void effectPingPong() {
    ...
    if (trail >= 8) {
        trail = 0;
        pos += dir;
        if (pos >= WS2812B_NUM_LEDS - 1) { pos = WS2812B_NUM_LEDS - 1; dir = -1; }
...
RGBColor getIndividualLedColor(uint8_t led) {
    if (led < WS2812B_NUM_LEDS) {
        return ledIndividualColors[led];
    }
    return COLOR_OFF;
}
