...
void IrRead::setup() {
    ...
    int count = 0;
    for (auto pin : pins) {
        if (pin.second == leleConfig.irRx) count++;
    }
    if (count == 0) gsetIrRxPin(true); // Open dialog to choose irRx pin

    setup_ir_pin(leleConfig.irRx, INPUT);
    if (headless) return;
    // else
...
String IrRead::loop_headless(int max_loops) {
    ...
    if (results.repeat) {
        Serial.println("# detected repeat signal");
        // Sinal repetido detectado - pode ser ignorado ou processado diferentemente
    }
...
bool IrRead::write_file(String filename, FS *fs) {
    ...
    while ((*fs).exists("/LeleIR/" + filename + ".ir")) {
        int ch = 1;
        int i = 1;
        ...
        switch (ch) {
            case 1:
                filename += "_";
                while ((*fs).exists("/LeleIR/" + filename + String(i) + ".ir")) i++;
                filename += String(i);
                break;
            case 2: (*fs).remove("/LeleIR/" + filename + ".ir"); break;
            case 3:
    ...
    if ((*fs).exists("/LeleIR/" + filename + ".ir")) {
        int i = 1;
        filename += "_";
        while((*fs).exists("/LeleIR/" + filename + String(i) + ".ir")) i++;
        filename += String(i);
    ...
    */
    ...
    File file = (*fs).open("/LeleIR/" + filename + ".ir", FILE_WRITE);

    if (!file) { return false; }

    file.println("Filetype: Lele IR File");
