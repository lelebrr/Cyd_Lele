...
bool txIrFile(FS *fs, String filepath, bool hideDefaultUI) {
    ...
    setup_ir_pin(leleConfig.irTx, OUTPUT);
    // digitalWrite(leleConfig.irTx, LED_ON);
        ...
        if (line.startsWith("type:")) {
            ...
            if (type == "raw") {
                ...
                while (databaseFile.available()) {
                    ...
                    } else if (line.indexOf("#") != -1 || !databaseFile.available()) {
                        // Detecta fim do comando por '#' ou EOF
    ...
    resetCodesArray();
    digitalWrite(leleConfig.irTx, LED_OFF);
    return true;
}
...
void otherIRcodes() {
    ...
    // select a file to tx
    if (!(*fs).exists("/LeleIR")) (*fs).mkdir("/LeleIR");
    filepath = loopSD(*fs, true, "IR", "/LeleIR");
...
void sendNECCommand(String address, String command, bool hideDefaultUI) {
    IRsend irsend(leleConfig.irTx); // Set the GPIO to be used to sending the message.
    irsend.begin();
    ...
    if (leleConfig.irTxRepeats > 0) {
        for (uint8_t i = 1; i <= leleConfig.irTxRepeats; i++) { irsend.sendNEC(data, 32); }
    }

    Serial.println(
        "Sent NEC Command" +
        (leleConfig.irTxRepeats > 0 ? " (1 initial + " + String(leleConfig.irTxRepeats) + " repeats)" : "")
    );
...
void sendRC5Command(String address, String command, bool hideDefaultUI) {
    IRsend irsend(leleConfig.irTx, true); // Set the GPIO to be used to sending the message.
    irsend.begin();
    if (!hideDefaultUI) { displayTextLine("Sending.."); }
    ...
    if (leleConfig.irTxRepeats > 0) {
        for (uint8_t i = 1; i <= leleConfig.irTxRepeats; i++) { irsend.sendRC5(data, 13); }
    }
    Serial.println(
        "Sent RC5 Command" +
        (leleConfig.irTxRepeats > 0 ? " (1 initial + " + String(leleConfig.irTxRepeats) + " repeats)" : "")
...
void sendRC6Command(String address, String command, bool hideDefaultUI) {
    ...
    if (leleConfig.irTxRepeats > 0) {
        for (uint8_t i = 1; i <= leleConfig.irTxRepeats; i++) { irsend.sendRC6(data, 20); }
    }
...
void sendSamsungCommand(String address, String command, bool hideDefaultUI) {
    ...
    if (leleConfig.irTxRepeats > 0) {
        for (uint8_t i = 1; i <= leleConfig.irTxRepeats; i++) { irsend.sendSAMSUNG(data, 32); }
    }
...
void sendSonyCommand(String address, String command, uint8_t nbits, bool hideDefaultUI) {
    ...
    if (leleConfig.irTxRepeats > 0) {
        for (uint8_t i = 1; i <= leleConfig.irTxRepeats; i++) { irsend.sendSony(data, nbits, 2); }
    }
...
void sendKaseikyoCommand(String address, String command, bool hideDefaultUI) {
    ...
    if (leleConfig.irTxRepeats > 0) {
        for (uint8_t i = 1; i <= leleConfig.irTxRepeats; i++) { irsend.sendPanasonic64(msb_data, 48); }
    }
...
bool sendDecodedCommand(String protocol, String value, uint8_t bits, bool hideDefaultUI) {
    ...
    if (hasACState(type)) {
        ...
        if (leleConfig.irTxRepeats > 0) {
            for (uint8_t i = 1; i <= leleConfig.irTxRepeats; i++) { irsend.send(type, state, state_pos); }
        }
        ...
        if (leleConfig.irTxRepeats > 0) {
            for (uint8_t i = 1; i <= leleConfig.irTxRepeats; i++) { irsend.send(type, value_int, bits); }
        }
    }
...
void sendRawCommand(uint16_t frequency, String rawData, bool hideDefaultUI) {
    ...
    if (leleConfig.irTxRepeats > 0) {
        for (uint8_t i = 1; i <= leleConfig.irTxRepeats; i++) {
            irsend.sendRaw(dataBuffer, count, frequency);
        }
    }
