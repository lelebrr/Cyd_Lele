...
bool Wardriving::begin_gps() {
    releasePins();
    GPSserial.begin(
        leleConfig.gpsBaudrate, SERIAL_8N1, leleConfigPins.gps_bus.rx, leleConfigPins.gps_bus.tx
    );
...
void Wardriving::append_to_file(int network_amount) {
    ...
    if (!(*fs).exists("/LeleWardriving")) (*fs).mkdir("/LeleWardriving");

    bool is_new_file = false;
    if (!(*fs).exists("/LeleWardriving/" + filename)) is_new_file = true;
    File file = (*fs).open("/LeleWardriving/" + filename, is_new_file ? FILE_WRITE : FILE_APPEND);
    ...
    if (is_new_file) {
        file.println(
            "WigleWifi-1.6,appRelease=v" + String(LELE_VERSION) + ",model=M5Stack GPS Unit,release=v" +
            String(LELE_VERSION) +
            ",device=ESP32 M5Stack,display=SPI TFT,board=ESP32 M5Stack,brand=Lele,star=Sol,body=4,subBody=1"
        );
        file.println(
...
void Wardriving::releasePins() {
    ...
    if (leleConfigPins.gps_bus.rx == nrf24ControlPin) {
