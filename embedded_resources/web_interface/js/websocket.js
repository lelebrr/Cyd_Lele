...
function initWebSocket() {
    ...
    WavePwn.ws.onclose = () => {
        ...
        if (reconnectAttempts <= 3 || reconnectAttempts % 5 === 0) {
            ...
            addLog('ConexÃ£o perdida. Reconectando...', 'warning');
        }

        updateConnectionStatus('reconnecting');
        showRateLimitedToast('ConexÃ£o Perdida!', 'error');
    ...
    WavePwn.ws.onerror = (error) => {
        ...
        if (reconnectAttempts <= 2) {
            ...
            addLog('Erro de conexÃ£o WebSocket', 'error');
        }
    };
    ...
    if (data.handshake_captured) {
        ...
        if (typeof sendDiscordNotification === 'function') {
            sendDiscordNotification(
                `ðŸŽ‰ Handshake capturado: **${data.handshake_captured.ssid}**`,
                'handshake'
            );
        }
    }
...
function updateNetworkTable(networks) {
    ...
    networks.forEach((net, index) => {
        ...
        tr.innerHTML = `
            ...
            <td>
                <button class="btn btn-sm btn-danger" onclick="startDeauth('${net.bssid}')">âš¡</button>
                <button class="btn btn-sm btn-primary" onclick="selectTarget('${net.bssid}', '${escapeHtml(net.ssid)}')">ðŸŽ¯</button>
            </td>
        `;
...
function showHandshakeNotification(data) {
    ...
    // Show notification
    addLog(`ðŸŽ‰ Handshake capturado: ${data.ssid}`, 'success');
