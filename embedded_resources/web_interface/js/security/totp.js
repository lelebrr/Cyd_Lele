...
function generateQRCodeData(secret, label) {
    const issuer = 'Lele';
    const uri = `otpauth://totp/${encodeURIComponent(label || 'Lele')}?secret=${secret}&issuer=${issuer}&algorithm=SHA1&digits=6&period=30`;
    return uri;
}
...
function enableTOTP() {
    ...
    const qrData = generateQRCodeData(secret, 'Lele');
