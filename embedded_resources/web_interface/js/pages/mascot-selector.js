...
async function loadMascotList() {
    try {
        ...
        MascotSelector.mascots = [
            ...
            { id: 1, type: 'lele', name: 'Lele', emoji: '👦', description: 'Garoto Wave' },
            { id: 2, type: 'lisa', name: 'Lisa', emoji: '👧', description: 'Garota Wave' },
            { id: 3, type: 'cat', name: 'Hackito', emoji: '🐱', description: 'Gato hacker' },
            { id: 3, type: 'robot', name: 'Robo-9', emoji: '🤖', description: 'Robô retro' }
        ];
    }
}
...
async function selectMascot(id) {
    try {
        ...
        // Sync with Main Dashboard (MascotSystem)
        const map = { 0: 'dragon', 1: 'lele', 2: 'lisa', 3: 'cat' };
        const slug = map[id] || 'dragon';

        if (window.MascotSystem) {
            window.MascotSystem.currentMascot = slug;
            localStorage.setItem('wavepwn-mascot', slug);
            window.MascotSystem.render(); // Updates main avatar

            // Boot/Sidebar now use static logo images (logo_loading.png / logo_dashboard.png)
            // The emoji update is disabled as these elements no longer exist
            // const emoji = window.MascotSystem.mascots[slug]?.emoji || '🐉';
            // const boot = document.getElementById('boot-mascot');
            // const side = document.getElementById('sidebar-mascot');
            // if (boot) boot.textContent = emoji;
            // if (side) side.textContent = emoji;
        }
