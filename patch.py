...
# Skip patching if framework is not installed yet
if FRAMEWORK_DIR is None or not exists(FRAMEWORK_DIR):
    print("[PATCH] Framework not installed yet, skipping patch...")
else:
    ...
    if not isfile(join(FRAMEWORK_DIR,mcu, "lib", ".patched")):
        ...
        if mcu=="esp32c5":
            env.Execute(
                '"%s" pkg exec -p toolchain-riscv32-esp -- riscv32-esp-elf-objcopy  --weaken-symbol=ieee80211_raw_frame_sanity_check %s %s'
                % (PIO_PATH, original_file, patched_file)
            )
        else:
            env.Execute(
                '"%s" pkg exec -p toolchain-xtensa-%s -- xtensa-%s-elf-objcopy  --weaken-symbol=ieee80211_raw_frame_sanity_check %s %s'
